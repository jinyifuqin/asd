...
{
    protected $auto = ['pro_img', 'pro_img_more','pro_parent_path'];
    ...
    public function getProUrlAttr($value, $data){
        return \think\Url::build('Product/item', 'id='.$data['id']);
    }
    ...
    public function setProImgMoreAttr($value){
     ...
     //父级路径
    public function setProParentPathAttr($value, $data)
    {        
        $data['pro_cate_id'] = @$data['pro_cate_id'] ? $data['pro_cate_id'] : '0';
        $parent_path       = $this->proCate()->where('id', $data['pro_cate_id'])->value('parent_path');
        return $parent_path;
    }
}
